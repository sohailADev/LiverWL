    ...
    function sendFile($summernote, file) {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#summernote').summernote('insertImage', 'http://learning.wyredinsights.com/'.concat(data.filePath)); //Directly insert the path, filename optional
                console.log(data.filePath);
            },
            error: function(data) {
                console.log(data);
                console.log("Failed to upload pictures!");
            },
        ...
        mutations.forEach(function(mutation) //     forEach: Traverses all MutationRecords
            {
                ...
                if (mutation.removedNodes[0] != null) {
                    ...
                    if (mutation.removedNodes[0].tagName == "IMG") {
                        ...
                        imgSrc = imgSrc.replace('http://learning.wyredinsights.com/', ''); //A stupid way to separate relative paths
   ...
   // $("#formSubmit").click(function () {
    ...
    $("#editPostForm").on('submit', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log(data.imageError);
                debugger;
                if (!data.bodyError && !data.imageError && !data.titleError)
                {
                    window.location.replace("http://learning.wyredinsights.com");
                }
                else
                {
                    window.location.replace("http://learning.wyredinsights.com/Posts/edit/" + data.postId);
...
$("#editPost").click(function () {
    ...
    $.ajax({
        ...
        success: function (id) {
            debugger;
            var nid = id[0];
            window.location.replace("http://learning.wyredinsights.com/Posts/edit/" + id);
